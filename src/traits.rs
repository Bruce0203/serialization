...
use crate::macros::{input_all_prmitives_of, input_num_types};

macro_rules! size_of_num_fn {
    ($($type:ty),*) => {$(
        concat_idents!(fn_name = size_of_, $type, {
            fn fn_name() -> bool {
                true
            }
        });
...
macro_rules! size_of_fn {
    ($($type:ty),*) => {
        fn size_of<T: 'static>() -> bool {
            #[allow(unused_unsafe)]
            unsafe {
                match type_id::<T>() {
...
#[const_trait]
pub trait PrimitiveTypeSizeChecker {
    input_num_types!(size_of_fn!());

    input_num_types!(size_of_num_fn!());
}

pub struct AllPrimitiveTypesAreSized;
impl const PrimitiveTypeSizeChecker for AllPrimitiveTypesAreSized {}

pub struct AllPrimitiveTypesAreUnsized;
impl const PrimitiveTypeSizeChecker for AllPrimitiveTypesAreUnsized {
    fn size_of<T: 'static>() -> bool {
        false
    }
}
...
pub trait Encoder: Sized + WriteBuf<u8> + const PrimitiveTypeSizeChecker {
...
pub trait Decoder: Sized + ReadBuf<u8> + const PrimitiveTypeSizeChecker {
